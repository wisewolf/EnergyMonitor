...
MCP3208 mcp3208(SPICLOCK,DATAOUT,DATAIN,SELPIN);
...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout)
{
  ...
  {
     startV = mcp3208.readADC(inPinV);                    //using the voltage waveform
  ...
  while ((crossCount < crossings) && ((millis()-start)<timeout))
  {
    ...
    //-----------------------------------------------------------------------------
    sampleV = mcp3208.readADC(inPinV);                 //Read in raw voltage signal
    sampleI = mcp3208.readADC(inPinI);                 //Read in raw current signal
...
double EnergyMonitor::calcIrms(unsigned int Number_of_Samples)
{
  ...
  {
    sampleI = mcp3208.readADC(inPinI);
